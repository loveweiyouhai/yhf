...
//        file_put_contents($this->path."/".$file.".php", json_encode(['date'=>date('Y-m-d H:i:s'),'message'=>$message]).PHP_EOL, FILE_APPEND);
//    }
//    public function logSaveYmd($message, $file = 'log', $year = false, $month = false, $day = false)
//    {
//
//        if (!is_dir($this->path)) {
//            mkdir($this->path, 777);
...
//
//        if ($month) {
//            if (!is_dir($this->path . "/" . date('m'))) {
//                mkdir($this->path . "/" . date('m'), 777, true);
//            }
//            $this->path = $this->path . "/" . date('m');
//        }
//
//        if ($day) {
//            if (!is_dir($this->path . "/" . date('d'))) {
//                mkdir($this->path . "/" . date('d'), 777, true);
//            }
//            $this->path = $this->path . "/" . date('d');
//        }
//
//        file_put_contents($this->path."/".$file.".php", json_encode(['date'=>date('Y-m-d H:i:s'),'message'=>$message]).PHP_EOL, FILE_APPEND);
//    }
